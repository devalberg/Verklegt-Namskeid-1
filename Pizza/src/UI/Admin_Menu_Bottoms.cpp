...
Admin_Menu_Bottoms::Admin_Menu_Bottoms()
{
    //ctor
}

void Admin_Menu_Bottoms::run()
{
    ...
    while (cont)
    {
        ...
        vector<Bottom> items = _service.view_items();
        if (!items.size())
        {
        ...
        else
        {
            ...
            {
                cout << left << "\tBottom name: " << setw(22) << items[i]  << "Shortcut: " << setw(5)  <<
...
bool Admin_Menu_Bottoms::get_command()
{
START:
    string input;
    cin >> input;
...
void Admin_Menu_Bottoms::add_item()
{
    bool cont = true;
    while (cont)
    {
        ...
        while(true)
        {
            ...
            try
            {
                _service.add_item(Bottom(name, identity, price));
...
void Admin_Menu_Bottoms::delete_item()
{
   bool cont = true;
    while (cont)
    {
